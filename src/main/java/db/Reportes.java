...
public class Reportes implements WithGlobalEntityManager {
	...
	public Map<Cliente, Double> consumoPorHogarEntre(LocalDateTime desde, LocalDateTime hasta) {
		...
		em.createQuery(
				"SELECT c.nombre, SUM(hc.consumo) consumoPorHogar FROM cliente c " + " INNER JOIN  c.dispositivos AS d"
						+ " INNER JOIN  d.consumoHastaElMomento AS td" + " INNER JOIN  td.consumoHastaElMomento AS hc"
						+ " WHERE hc.fecha <= :desde AND hc.fecha >= :hasta" + " GROUP BY d.id, c.id_cliente")
				.setParameter(0, desde).setParameter(1, hasta).getResultList();
