...
public class Reportes implements WithGlobalEntityManager{
	...
	public Map<Cliente, Double> consumoPorHogarEntre(LocalDate desde, LocalDate hasta) {
		...
		/*
		 * SELECT c.nombre, SUM(hc.consumo) consumoPorHogar
		 * FROM cliente c 
	...
	public Map<Dispositivo, Double> consumoPromedioPorDispositivoDe() {
		...
		/* 
		 * SELECT c.nombre, d.nombre, AVG(hc.consumo) promedio
		 * FROM cliente c 
	...
	public Map<Transformador, Double> consumoPorTransformadorEntre(LocalDate desde, LocalDate hasta) {
